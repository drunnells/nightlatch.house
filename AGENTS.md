...
- The player client must remain polished and playable on mobile and desktop. Use the bottom-right narration tray for both transient rule messages and explicit room/object descriptions: transient text dismisses automatically, while Describe text remains until toggled closed or superseded by gameplay text without moving focus or shifting the viewport. Preserve the complete content image in the available viewport, center Describe along the artwork's top edge, and keep fullscreen at the top-left. Center the desktop Behind you control above the footer hint (or at the bottom in fullscreen), preserve its mobile position, and use that single control for contextual and Gateway returns instead of a separate Gateway button.
...
- Room and object region array order is the saved back-to-front layer order for editor shapes, runtime hit regions, and overlays. Authors drag sidebar regions lower to bring them forward, or use Alt + Up / Down on a focused region; reordering marks the draft dirty and preserves stable IDs.
- Non-door regions may opt into `clickRequiresOverlay`: ordinary clicks and keyboard activation are accepted only while their own region-scoped overlay is present and they have authored click actions. Clearing the overlay makes that region passive so underlying regions can receive clicks. This setting defaults off, does not affect automatic or inventory Use behaviors, and never disables doors.
...
- The debug-play page is an authoring tool. It should fit the complete room into the available viewport, keep transient player messages in the desktop runtime inspector instead of over interaction canvases, traverse canonical static connections, present a named Behind you control for contextual and Gateway returns, keep randomized Gateway assignments stable until reset, and let designers inspect matched branches, condition traces, executed results, messages, overlays, flags, items, inventory objects, unlocked doors, arrival behavior, Gateway assignments, and the event log.
...
- The browser alone performs the existing authenticated save and image-generation requests. Save, discard, and overlay-generation actions must stay visibly human-approved in the Agent session panel. An explicit user request to make, create, or generate a room/object image authorizes the MCP to immediately run the normal Gemini background workflow rather than asking for approval; it must visibly populate the prompt, click the editor's background-generation control, keep its result unsaved for review, and continue using `assets/js/room-editor.js` and the existing shared rule semantics.
...
- Book page generation lets authors choose the full object canvas, the page's current overlay, or any earlier populated page as Gemini's reference. Blank pages default to the object and regenerations default to the current page. An earlier page is a design reference for new content rather than an edit target; every source asset remains unchanged until normal cleanup. Store the author prompt with the generated page while keeping prompts and reference-picker state out of the public catalog.
