...
- An object may opt into built-in book behavior through `data.book`. A book reserves two existing object interaction region IDs for previous/next navigation and stores up to 100 ordered full-canvas page overlay assets. Those two regions bypass click logic while book behavior is enabled, but their automatic behaviors remain active; every other object region keeps normal declarative rule behavior.
