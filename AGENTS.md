...
- An object may opt into built-in book behavior through `data.book`. A book reserves two existing object interaction region IDs for previous/next navigation, stores up to 100 ordered full-canvas page overlay assets with author-only Gemini prompts, and may select one saved sound slug used for every successful page turn. Those two regions bypass click logic while book behavior is enabled, but their automatic behaviors remain active; every other object region keeps normal declarative rule behavior.
...
- Book page generation uses the full object canvas as Gemini's initial reference. Regenerating a page uses that page's current overlay as the exact reference, preserves the prior temporary or saved asset until normal cleanup, and stores the author prompt with the page while keeping it out of the public catalog.
