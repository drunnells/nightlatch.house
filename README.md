...
The player client opens on an outside-house entry screen, then begins a new run in the entry room of the cluster marked **Start** on the Map or resumes the saved browser run. It uses the same authored room, object, topology, automatic-behavior, sound, and rule data as debug play. **Start over** clears progress and rerolls Gateway assignments while staying inside; confirmed **Exit game** clears the run and returns to the entry screen. Transient rule messages and explicit room/object descriptions share the bottom-right narration tray; transient text dismisses automatically, while Describe text remains until toggled closed or replaced by gameplay text. Describe is centered over the artwork, fullscreen remains at the upper-left, and the unified object viewer keeps close, sound, inventory, and menu controls visible around its scaled interaction canvas.
