...
The player client opens on an outside-house entry screen, then begins a new run in the entry room of the cluster marked **Start** on the Map or resumes the saved browser run. It uses the same authored room, object, topology, automatic-behavior, sound, and rule data as debug play. **Start over** clears progress and rerolls Gateway assignments while staying inside; confirmed **Exit game** clears the run and returns to the entry screen. Transient rule messages slide in from the right and dismiss automatically, while explicitly opened room/object descriptions stay until closed without shifting the interaction viewport. Description and fullscreen controls sit directly on room and object artwork, and the unified object viewer keeps close, sound, inventory, and menu controls visible around its scaled interaction canvas.
